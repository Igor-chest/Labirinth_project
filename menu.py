...
def main_menu(sections):  # sections=((name1,function1,0/underSections),...,(name4,function4,0/underSections))
    ...
    DrawText('Лабиринт', font, surface_menu, 300, 25)
            ...
            if iwrite.type == pygame.KEYDOWN:
                ...
                if iwrite.key == pygame.K_BACKSPACE:
                    if text.__len__():
                        ...
                        DrawText('(<= 100 и >= 5)', font, surface_menu, 320, 280)
                        font = pygame.font.Font(None, 40)
    ...
    while done_mode:
        ...
        if mode == 0:
            ...
            if window:
                ...
                DrawText('(<= 100 и >= 5)', font, surface_menu, 320, 280)
                text = list(write_mode('введите размер лабиринта', surface_menu))

                done_mode_check = True
                i_mode_check = 0
                while done_mode_check:
                    if (i_mode_check <= len(text)) and (text[i_mode_check] in numbers):
                        i_mode_check += 1
                        if i_mode_check == len(text):
                            break
                    else:
                        done_mode_check = False

                text = ''.join(text)

                if done_mode_check and int(text) <= 100 and int(text) >= 5:
                    text = int(text)
